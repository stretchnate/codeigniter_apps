<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	function error($var, $str_left="", $str_right="") {
		...
		error_log($error, 3 ,'/var/www/source/CI_3.1.6/application/logs/debug.log');
	}

	function clean($log = "/var/www/source/CI_3.1.6/application/logs/debug.log") {
	...
	function debugOut($var, $mode = "a", $indent = "\n") {
		if( is_array($var) || is_object($var) ) {
			pre($var, $mode, "/var/www/source/CI_3.1.6/application/logs/debug.log", $indent);
		}
		else {
			writeToFile($var, $mode, '/var/www/source/CI_3.1.6/application/logs/debug.log', $indent);
		}
	}
	...
	function dbo($var, $mode = "a", $recurse = false) {
		if( is_array($var) ) {
			...
			//dbo_obj needs some work, it recurses too far when called from here.
			// dbo_obj($var, $mode, $recurse);
			dbo("object detected");
			$var = get_class($var);
			dbo($var);
			// dbo("object detected, check dnate_arr.log");
			// errDump($var, 'a');
		} else {
			debugOut($var, $mode);
		}
	...
	function dbo_obj($class, $mode = 'a', $recurse = false, $indent = "\n") {
			...
			if(strpos($method, 'get') !== false) {
				...
				if($reflection->getMethod($method)->getNumberOfParameters() > 0) {
					...
					if(is_string($return_value)) {
						$txt .= $indent."\t" . $reflection->getName() .'::'. $method.' = '.$return_value;
					}
					else if(is_array($return_value)) {
						$txt .= $indent."\t" . $reflection->getName() .'::'. $method;
						$txt .= $indent."\t\t" . getArrayKeysAndValues($return_value, '', $indent."\t\t");
						// dbo_arr($reflection->getName() .'::'. $method, $return_value, $mode);
					}
					else if(is_object($return_value)) {
						if($recurse === true) {
							// dbo($method, $mode);
							// dbo_obj($return_value, 'a', $recurse, $indent . "\t");
							$txt .= $indent."\t" . $reflection->getName() .'::'. $method;
							$txt .= dbo_obj($return_value, 'a', $recurse, $indent . "\t");
						} else {
							$txt .= $indent."\t" . $method . " returns an object of " . get_class($return_value);
						}
					}
				}
			}
	 ...
	 * @author dnate
	 */
	function pre(&$array, $mode = 'w', $filename = "/var/www/source/CI_3.1.6/application/logs/debug.log", $indent = "\r\n") {
	...
	function label($label, $file = "dbo", $mode = 'a') {
		switch($file) {
			case "dump":
				$file = '/var/www/source/CI_3.1.6/application/logs/debug.log';
				break;
	 ...
	 * @author dnate
	 */
	function errDump($var, $mode = 'w', $file = '/var/www/source/CI_3.1.6/application/logs/debug.log') {
		ob_start();
	 ...
	 * @author dnate
	 */
	function writeToFile($string, $mode = 'a', $filename = '/var/www/source/CI_3.1.6/application/logs/debug.log', $indent = "\n") {
		if(is_array($string)) {
			if($filename == "/var/www/source/CI_3.1.6/application/logs/debug.log") {
				$filename = "/var/www/source/CI_3.1.6/application/logs/debug.log";
			}
	...
	* @author dnate
	*/
	function wtf($string, $mode = 'a', $filename = '/var/www/source/CI_3.1.6/application/logs/debug.log', $indent = "\n") {
